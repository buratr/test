<script  lang="ts">
	...
	import {getFirestore, collection, getDocs, query, limit, orderBy, startAfter, addDoc } from "firebase/firestore"; 
	...
	let inputEmail:string, inputName:string, inputSubject:string, inputMessage:string
	let errFill:boolean = false, contactSubmited:boolean = false;
	...
	$: totalAmount = getTotalOrderAmount($cartItems || []);
	
	let selectedTransfer = 'Direct'; // Первоначально выбрана "Direct Bank Transfer"

    // Функция для обработки изменений выбора радиобатонов
    function handleRadioChange(event:any) {
		//console.log(event)
        selectedTransfer = event.target.value;
    }
</script>
...
<section class="flex bg-cover justify-center bg-center relative">
	...
	<div class="container z-10">
		<div class="flex justify-center py-36">
			<div class=" ">
				<div class="font-['Poppins'] text-5xl font-medium text-[#000000] ">Checkout</div>
				<div class="flex justify-center mt-3">
					...
					<div class="font-['Poppins'] text-base font-medium text-[#000000]">Checkout</div>
				</div>
			</div>
		</div>
	</div>
...
<section class="flex justify-center mt-24 mb-20">
	<div class="container">
	{#if !contactSubmited}
			<form class="flex flex-row h-full" method="post" on:submit|preventDefault={formContact}>
				<!-- Contact form  -->
				<div class="flex flex-col w-3/6 ml-12 mr-12 items-end">
					
						<div class="shrink w-full max-w-lg items-end" >
							<div class="grid grid-cols-2 gap-x-5  mb-9 ">
								<div>
									<label for="name" class="font-['Poppins'] text-base font-medium text-[#000000]">First Name</label>
									<input bind:value={inputName} name="name" id="name" type="text" placeholder="Abc" 
									class="mt-5  w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">
								</div>
				
								<div>
									<label for="lname" class="font-['Poppins'] text-base font-medium text-[#000000]">Last Name</label>
									<input bind:value={inputName} name="lname" id="lname" type="text" placeholder="Abc" 
									class="mt-5 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">	
								</div>
							</div>

							<label for="cname" class="font-['Poppins'] text-base font-medium text-[#000000]">Company Name (Optional)</label>
							<input bind:value={inputName} name="cname" id="cname" type="text" placeholder="Abc" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="Country" class="font-['Poppins'] text-base font-medium text-[#000000]">Country / Region</label>
							<input bind:value={inputSubject} name="Country" id="Country" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="Street" class="font-['Poppins'] text-base font-medium text-[#000000]">Street address</label>
							<input bind:value={inputSubject} name="Street" id="Street" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="City" class="font-['Poppins'] text-base font-medium text-[#000000]">Town / City</label>
							<input bind:value={inputSubject} name="City" id="City" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="Province" class="font-['Poppins'] text-base font-medium text-[#000000]">Province</label>
							<input bind:value={inputSubject} name="Province" id="Province" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="ZIP" class="font-['Poppins'] text-base font-medium text-[#000000]">ZIP code</label>
							<input bind:value={inputSubject} name="ZIP" id="ZIP" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="Phone" class="font-['Poppins'] text-base font-medium text-[#000000]">Phone</label>
							<input bind:value={inputSubject} name="Phone" id="Phone" type="text" placeholder="This is an optional" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<label for="email" class="font-['Poppins'] text-base font-medium text-[#000000]">Email address</label>
							<input bind:value={inputEmail} name="email" id="email" type="text" placeholder="Abc@def.com" 
							class="mt-5 mb-9 w-full h-16 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black ">

							<textarea bind:value={inputMessage} name="Message" id="Message"  placeholder="Additional information" 
							class="mt-5 mb-9 w-full h-32 p-2 rounded outline-none border-[1px] border-slate-400 focus-visible:border-black "/>
				...
				<!-- info wrap -->
				<div class="h-full w-3/6 ml-12 mr-12">
					<div class="sticky top-10 flex flex-col  items-end">
						<div class="w-full border-b-[1px] mb-6">
							<div class="flex justify-between mb-5">
								<div class="font-['Poppins'] text-2xl font-medium text-black">Product</div>
								<div class="font-['Poppins'] text-2xl font-medium text-black">Subtotal</div>
							</div>
							...
							<div class="flex justify-between mb-5">
								...
								<div class="font-['Poppins'] text-base font-normal text-black">Rs. {item.price * item.quantity}</div>
							</div>
							{/each}
							<!-- total -->
							<div class="flex justify-between mb-8">
								<div class="font-['Poppins'] text-base font-normal text-black">Total</div>
								<div class="font-['Poppins'] text-2xl font-bold text-[#B88E2F]">Rs. {totalAmount||0}</div>
							</div>
						</div>

						<div class="w-full ">
							<div class="mb-4">
								<input type="radio" name="transfer" id="Direct" checked value="Direct" on:change={handleRadioChange}/>
								<label class="font-['Poppins'] text-base font-normal {selectedTransfer === 'Direct' ? 'text-black' : 'text-[#9F9F9F]'}" for="Direct">Direct Bank Transfer</label>
							</div>
							<div class="mb-6 font-['Poppins'] text-base font-light text-[#9F9F9F]">
								Make your payment directly into our bank account. Please use your Order ID as the payment reference. Your order will not be shipped until the funds have cleared in our account.
							</div>

							<div class="mb-4">
								<input type="radio" name="transfer" id="Cash" value="Cash" on:change={handleRadioChange}/>
								<label class="font-['Poppins'] text-base font-normal {selectedTransfer === 'Cash' ? 'text-black' : 'text-[#9F9F9F]'}" for="Cash">Cash On Delivery</label>
							</div>
							<div class="mb-6 font-['Poppins'] text-base font-light text-black">
								Your personal data will be used to support your experience throughout this website, to manage access to your account, and for other purposes described in our privacy policy.
							</div>
						</div>
					
						{#if errFill}
							<div class="font-['Poppins'] text-base font-medium text-red-500">Fill in all the fields</div>
						{/if}
						<button type="submit" 
						class="mx-auto w-60 rounded-2xl text-black mt-5 p-3 border-[1px] border-black bg-white hover:bg-[#d9b051] hover:text-white hover:border-[#d9b051]">
							Place order
						</button>
			...
			{:else}
						<div class="mx-auto rounded p-20 font-['Poppins'] text-xl font-medium text-black border-[1px] border-slate-400">
