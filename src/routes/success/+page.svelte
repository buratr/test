<script  lang="ts">
	...
	// export { load };
	//export let uid;
	import { cartItems, removeFromCart, updateCartItemQuantity, getTotalOrderAmount, updateCartItemPaid } from '$lib/store';
	...
	if (typeof window !== 'undefined') {
		...
		pid = JSON.parse(pid)
		console.log("uid: --- : ", uid);
		if(uid.indexOf('guest')>-1){
			pid.forEach((id: any)=>{
				console.log(id);
				updateCartItemPaid(id, true)
			})
		}else{
		...
		<div class="flex flex-row justify-between w-full">
			<div class="flex-1">
				...
				<div class="mt-14">
					...
					<div class="flex justify-between items-center mb-8 {item.paid?'bg-lime-200':' bg-red-200'}">
						<div class="relative">
